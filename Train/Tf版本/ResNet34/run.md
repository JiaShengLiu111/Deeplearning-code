    ...
    with tf.Session(config = config) as sess:    
        ...
        for i in range(0,STEPS):   # 表示从索引为start开始训练样本（主要是为了均衡二次训练时样本被训练的机会）
            ...
            if i%(int(cfg0.saveStepEpochRate*epoch))==0 and (i!=0 or cfg0.firstPred==True):  # 判断在进行训练之前是否进行预测。 
                ...
                train_predicts,_ = utils.predicts(sess,model.prob,model.inputs,model.is_training,X_train_evalua,batch_size,dataenhance,reDir)  
                val_predicts,_ = utils.predicts(sess,model.prob,model.inputs,model.is_training,X_val,batch_size,dataenhance,reDir)
